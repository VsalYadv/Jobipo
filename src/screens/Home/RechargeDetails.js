...
const RechargeDetails = ({ navigation, route }) => {
  const rechargeData = [
    {
      ...
      sms: '100 SMS/day',
      extraPoints: ['Free OTT subscription', 'Unlimited free SMS for the first 15 days', 'Recharge with ₹199 and get ₹10 cashback on your next recharge', 'unlimited 5G data for eligible subscribers'],
    },
    {
      id: '2',
      plan: '₹399',
      ...
      voice: 'Unlimited Calls',
      sms: '100 SMS/day',
      extraPoints: ['Access to premium content', 'Unlimited free calls to any network across India', 'Get 2GB additional high-speed data every day for 30 days'],
    },
    {
      id: '3',
      plan: '₹599',
      ...
      voice: 'Unlimited Calls',
      sms: '100 SMS/day',
      extraPoints: ['Free caller tunes', 'International roaming benefits', 'Get 1000 free SMS every month', 'Recharge with ₹599 and get ₹50 cashback on your next recharge'],
    },
    {
      id: '4',
      plan: '₹999',
    ...
    useCallback(() => {
      const GetDataFunc = async () => {
        ...
        if (sliderDataApi.status === 1) {
          ...
          setOperator(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).operator));

          setSelectedTab(Object.keys(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).info)[0]);
  ...
  const createOrder = async () => {
    const res = await fetch('https://api-preprod.phonepe.com/apis/pg-sandbox/checkout/v2/sdk/order', {
      method: 'POST',
      body: JSON.stringify({
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.cardDetails}>
        <Text style={styles.plan}>Rs.{item.rs}</Text>
        <View>
          <Text style={styles.validity}>{item.validity == "N/A" ? 'Current Plan' : item.validity}</Text>
      ...
      <View style={styles.buttonRow}>
        <TouchableOpacity style={styles.buyButton} onPress={() => handlePlan(item)}>
  ...
  const plansToShow = selectedTab && filteredData?.info
    ...
    <>
      ...
      <View style={styles.container}>
        <View style={styles.searchBarContainer}>
          <Ionicons name="search-outline" size={25} color="#666" style={styles.searchIcon} />
          <TextInput
                ...
                style={[
                  {
                    ...
                    borderBottomColor: selectedTab === item ? '#0d4574' : 'transparent',
                  }
                ]}
              >
                <Text style={{ color: selectedTab === item ? '#0d4574' : '#333', fontWeight: selectedTab === item ? 'bold' : 'normal' }}>
                  {item}
                </Text>
              </TouchableOpacity>
            )}
