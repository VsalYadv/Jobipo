 ...
 useFocusEffect(
  useCallback(() => {
    ...
    if (data.languageKnown) {
      ...
      setSelectedLanguages(langArray.filter(item => !item.startsWith('English:')));
    }
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        ...
        <View >
         <Text style={{ fontSize: 16, fontWeight: '700', color: '#000', marginBottom: 6,marginLeft:4, }}>
    Language Known
  </Text>
...
<View style={styles.cardlView}>
  ...
  {['Basic Speaking', 'Intermediate Speaking', 'Fluent Speking'].map((level) => (
    <TouchableOpacity
      key={level}
      style={[styles.radioBtn, englishLevel === level && styles.radioBtnActive]}
      onPress={() => setEnglishLevel(level)}
    >
      <Text style={[styles.radioText, englishLevel === level && styles.radioTextActive]}>{level}</Text>
    </TouchableOpacity>
  ))}
</View>
</View>
  ...
  <Text style={{ fontSize: 16, fontWeight: '700', color: '#000', marginVertical: 10,marginLeft:4, }}>
    Languages Known
  </Text>
          ...
          {languages.map((language, index) => (
            <View key={index} style={styles.languageItem}>
              ...
              <TouchableOpacity
                style={[styles.checkbox, selectedLanguages.includes(language) && styles.checked]}
                onPress={() => handleLanguageSelection(language)}
              >
                {selectedLanguages.includes(language) && <Icon name="check" size={20} color="#fff" />}
              </TouchableOpacity>
            </View>
          ))}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  saveButton: {
    marginVertical: 20, backgroundColor: "#0d4574", borderRadius: 10,
  },
  saveButtonText: {
