...
useFocusEffect(
  useCallback(() => {
    ...
    console.log("educationLevel data:", data?.educationLevel);
    console.log("Index:", index);
    console.log("Data is array:", Array.isArray(data));
    console.log("Data length:", data?.length);

    if (Array.isArray(data) && index != null && index < data.length) {
      console.log("educationLevel from backend:", data[index].educationLevel);
      setEducationData(data[index]); 
    } else if (data && typeof data === 'object') {
      console.log("educationLevel from backend:", data.educationLevel);
      setEducationData(data); 
    } else {
      console.log("Invalid index or data");
    }
  }, [data, index])
);
...
const showDegreePicker = ["Graduate", "Post Graduate"].includes(educationData.educationLevel);
...
const degreeOptions =
  ...
  const handleSubmit = async () => {
         console.log('Submitting education form data:', educationData);

    const d = parseIfArrayString(jobSeekerData.education)

    const updatedEducationData = index !== null ?
      d?.map((item, id) => id == Number(index) ? educationData : item) : [
        ...d,
        educationData
      ];
...
<SimpleHeader title="Update Education" titleColor="#585858" />
<ScrollView style={{ backgroundColor: '#F5F4FD' }}>
        <View style={styles.container}>
          <Text style={styles.label}>Qualification</Text>
          <View style={styles.buttonGroup}>
            {[" 10th Below","10th","12th","Graduate", "Post Graduate", "ITI","DIPLOMA" ].map((level) => (
              <TouchableOpacity
                key={level}
                style={[
                  styles.button,
                  educationData.educationLevel === level && styles.buttonSelected,
                ]}
                onPress={() =>
              ...
              >
                <Text
                  style={[
                    styles.buttonText,
                    educationData.educationLevel === level && styles.buttonTextSelected,
                  ]}
                >
                  {level}
                </Text>
...
<View style={styles.pickerWrapper}>
  ...
  >
    ...
    {degreeOptions.map((deg) => (
      <Picker.Item key={deg.value} label={deg.label} value={deg.value} />
    ))}
  </Picker>
</View>
