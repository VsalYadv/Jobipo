...
import { BuildingIcon, LocationIcon,BackArrowIcon,HomeIcon,InfoIcon,SettingsIcon,
  RefreshEyeIcon,NetworkIcon,GoogleIcon,LockShieldIcon,SvgHatIcon,AddDocumentIcon,
  JobReferenceIcon,JobUserNetworkIcon,JobMultipleNetworkIcon,JobAnalyticsBarGraphIcon,
JobProfileUserIcon,JobBuildingProfileIcon,JobWorkIcon,LocationSmallIcon,ViewIcon
 } from '../JobSvgIcons';
...
const JobDes = ({ route }) => {
  ...
  //console.log("chckk",job)

  // const { id } = route.params;
  //     console.log('Received Job in jobId:', job.jobId);
  

 const statusIndex = Math.max(0, Number(job.status) - 1); 
//   console.log("raw status:", job.status, "type:", typeof job.status);
 console.log("calculated statusIndex:", statusIndex);
// console.log("applicationStatus length:", applicationStatus.length);
// console.log("applicationStatus[statusIndex]:", applicationStatus[statusIndex]);
  const statusObj = applicationStatus[statusIndex] || { 
  title: "Unknown", 
...
useEffect(() => {
  ...
  const handleSave = () => {
    setIsSaved(!isSaved);
    Alert.alert('Job Saved', isSaved ? 'Job unsaved!' : 'Job saved successfully!');
  };

const salary = job.salaryType === 'Incentive-Based'
  ...
  return (
    ...
    >
            ...
            {job?.applicationStatus !== null && <View style={{
              paddingHorizontal: 10, paddingVertical: 10,
              flexDirection: 'row',
              width: '100%',
              alignItems: 'center',
              justifyContent: 'center',
             backgroundColor: statusObj?.style?.backgroundColor || 'gray', 
            }}>
              <Text style={{
                  color: statusObj?.style?.color || 'black',
              }}>{applicationStatus[Number(job.applicationStatus) - 1].title}</Text>
            </View>}
          </View>
  ...
  <View style={styles.sectionRow}>
    ...
    <Text style={[styles.location, { marginLeft: 12 }]}>
       {job?.companyAddress ? job.companyAddress : 'NA'}
    </Text>
  </View>
  <View style={styles.sectionRow}>
    <HomeIcon  />
 ...
 {job?.MinAge ? job.MinAge : 'N/A'}{job?.MaxAge ? ` to ${job.MaxAge}` : ''} Years
            </Text>
            </View>
            <View style={styles.sectionRow}>
   ...
   <View style={styles.jobCard}>
                  ...
                  ? Array.isArray(job.langaugeRequirenments)
                    ...
                    : (() => {
                        try {
                          ...
                          return Array.isArray(parsed) ? parsed.join(', ') : 'NA';
                        } catch {
                          return 'NA';
                        }
                      })()
            ...
            {(() => {
              try {
                ...
                return skills?.map((req, index) => (
                  <Text key={index} style={styles.skillItem}>• {req}</Text>
                ));
              } catch (error) {
                console.warn("Invalid JSON in job.skillsRequired:", error);
       ...
       <View style={styles.jobCard}>
            ...
            <Text style={[styles.location, { marginLeft: 12 }]}>
            <Text style={{ color:'#FF8D53' }}>Roles and Responsibilities :  </Text>
 ...
 <View style={styles.jobCard}>
            ...
            {job?.officeLocation ? job.officeLocation : 'NA '}
            </Text>
            </View>
    ...
    try {
      ...
      return <Text style={styles.benefitItem}>No valid benefits found.</Text>;
          ...
          <View style={styles.buttonContainer}>
            ...
            {isApplied && (job.contactType == 'WhatsApp & Call' || job.contactType == 'WhatsApp Only') &&
              <TouchableOpacity style={[styles.applyButton,
              {
                flex: 1,
                alignItems: 'center',
                ...
                onPress={() => {
                  Linking.openURL(`https://api.whatsapp.com/send?phone=${job.cWhatsappNo}`)
                }}
              >
                <Ionicons name="logo-whatsapp" size={20} color="#fff" />
              </TouchableOpacity>}

            {isApplied && (job.contactType == 'WhatsApp & Call' || job.contactType == 'Call Only') &&
              <TouchableOpacity style={[styles.applyButton,
              {
                flex: 1,
                alignItems: 'center',
