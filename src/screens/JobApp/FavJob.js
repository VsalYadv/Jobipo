...
const applicationStatusMeta = [
  ...
  { title: 'Interview Invite',  style: { backgroundColor: '#FFA500', color: '#fff' } },
...
const getStatus = (statusId) => {
  ...
  return applicationStatusMeta[idx] || {
    title: 'Unknown',
...
const FavJob = ({ navigation }) => {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchApplied = async () => {
        try {
          const res = await fetch('https://jobipo.com/api/Agent/myapplied', { method: 'GET' });
  ...
  const renderJobItem = ({ item }) => {
    ...
    const statusId = appliedStatusMap[item.jobId]; // ✅ only if applied
    ...
    return (
      <View style={styles.jobBox}>
        ...
        {/* ✅ Status sirf tab dikhayenge jab job applied bhi ho */}
        {statusMeta && (
    ...
    <>
      ...
      <View style={styles.container}>
        {isLoading ? (
          ...
          <FlatList
            data={favoriteJobs}
            keyExtractor={(item) => item.jobId?.toString() ?? Math.random().toString()} // ✅ better key
            renderItem={renderJobItem}
            ListEmptyComponent={
              <View style={{ alignItems: 'center', marginTop: 40 }}>
                <Icon name="favorite-border" size={50} color="#ccc" />
                <Text style={styles.noJobs}>You haven't favorited any jobs yet.</Text>
              </View>
            }
          />
        )}
