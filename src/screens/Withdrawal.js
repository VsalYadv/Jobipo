            ...
            const GetDataFunc = async () => {
              ...
              if (sliderDataApi ) {
                if(sliderDataApi.logout != 1){
                  ...
                  setBankName(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.BankName);
                  setAccountNo(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.AccountNo);
                  setBranchName(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.BranchName);
                  setAccountHolderName(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.AccountHolderName);  
                  setupdatedAt(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.updatedAt);  
                  setIfscCode(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.IfscCode);  
                  setisLoading(false);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.card}>
       <Picker
        ...
        <TouchableOpacity style={styles.saveButton} onPress={() => UpdatePayment() }>
      ...
      <View style={styles.withdrawSection}>
        ...
        <Text style={styles.withdrawLabel}>Withdrawal from Jobipo Wallet</Text>
        ...
        <TouchableOpacity style={styles.proceedButton} onPress={handleWithdrawal}>
          <Text style={styles.proceedButtonText}>Proceed to withdrawal ₹ {amount}</Text>
        </TouchableOpacity>
        <View style={styles.withdrawalContainer}>
        <Text style={styles.withdrawalText}>• Minimum withdrawal amount ₹ {amount}</Text>
        <Text style={styles.withdrawalText}>• Amount will be creadited in your bank account 24 to 48 working hours.(Saturday, Sunday and national holiday's withdrawal request proceed to next working day))</Text>
        </View>
      </View>
      </View>
    </ScrollView>
    <Modal
      ...
      >
        ...
        <RadioButton
          ...
          />
          <Text style={styles.transferOptionText}>Transfer my money to this account.</Text>
        </View>
