...
useFocusEffect(
  ...
  if (text.length >= 1) {
    const filtered = jobTitles.filter((item) => {
      console.log('ItemReg:', item); // 🧪 check each object
      if (!item.jobTitle) {
  ...
  return (
    <ScrollView  contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center',backgroundColor:'#F5F4FD', }}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.ContainerDetails}>
            ...
            {/* {formData.current_location !== '' && (
              <Text style={{ marginBottom: 10 }}>{formData.current_location}</Text>
            )} */}
            <GooglePlacesInput
