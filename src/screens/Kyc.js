...
const Kyc = ({ navigation }) => {
      ...
      const GetDataFunc = async () => {
        ...
        if (sliderDataApi) {
          if (sliderDataApi.logout != 1) {
            ...
            setNamePan(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.NamePan);
            setPanNo(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.PanNo);
            setupdatedAt(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.updatedAt);
            setKycStatus(JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.ProfileStatus);
                // console.log('ProfileStatus:', users?.ProfileStatus);


            if (JSON.parse(JSON.parse(JSON.stringify(sliderDataApi)).msg).users.ProfileStatus == 1) {
  ...
  const verifypan = async () => {
    ...
    try {
      ...
      const asd = await fetch(
        'https://jobipo.com/api/Singup/panVerify',
        {
          method: 'POST',
          headers: {
  ...
  const sendotp = async () => {
    ...
    try {
      ...
      const asd = await fetch(
        'https://jobipo.com/api/Singup/adhaarOtp',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const adhaarVerify = async () => {
    setisLoading(true);
    try {
      var formdat = { 'AdharNo': AdharNo, 'kycId': kycId, 'Tokan': Tokan, 'AdharOTP': AdharOTP };

      const asd = await fetch(
        'https://jobipo.com/api/Singup/adhaarVerify',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    <>
      ...
      <ScrollView style={styles.container}>
        <View style={styles.MainContainer}>
          <View style={[(KycStatus == 2 || KycStatus == 0) ? {
            flex: 1,
          } : {
            display: 'none',
          },
                ...
                <View style={styles.boxContainer}>
                  ...
                  <View style={styles.rightBox}>
                    <Image
                      ...
                      style={{ width: 130, height: 130, marginTop: 10, resizeMode: 'contain' }}
                    />
                  </View>
                </View>
              </View>
        ...
        <View style={styles.MainContainer}>
          <View style={[(KycStatus == 3 && OTPsent == 1) ? {
        ...
        <View style={styles.MainContainer}>
          <View style={[(KycStatus == 3 && OTPsent == 2) ? {
